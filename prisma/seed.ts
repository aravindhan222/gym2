...
async function main() {
  ...
  const class1 = await prisma.gymClass.create({
    data: {
      ...
      endTime: new Date(tomorrow.setHours(6, 45, 0, 0)),
    },
  });
  ...
  await prisma.classBooking.create({
    data: { userId: member1.id, classId: class1.id },
  });

  await prisma.classBooking.create({
    data: { userId: member2.id, classId: class1.id },
  });

  await prisma.classBooking.create({
    data: { userId: member2.id, classId: class2.id },
  });

  await prisma.classBooking.create({
    data: { userId: member3.id, classId: class3.id },
  });

  await prisma.classBooking.create({
    data: { userId: member4.id, classId: class4.id },
  });

  await prisma.classBooking.create({
    data: { userId: member5.id, classId: class1.id },
  });

  await prisma.classBooking.create({
    data: { userId: member4.id, classId: class2.id },
  });
